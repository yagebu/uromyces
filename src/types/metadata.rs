...
use pyo3::types::{PyBool, PyDate, PyDict, PyInt, PyNone, PyString, PyType};
...
use crate::types::decimal::get_decimal_decimal;
...
#[derive(Clone, Debug, PartialEq, Eq, Hash, Serialize, Deserialize, IntoPyObjectRef)]
#[serde(untagged)]
pub enum MetaValue {
    String(String),
    Account(Account),
...
impl<'py> FromPyObject<'_, 'py> for MetaValue {
    type Error = PyErr;

    fn extract(obj: Borrowed<'_, 'py, PyAny>) -> Result<Self, Self::Error> {
        if let Ok(string) = obj.cast::<PyString>() {
            return Ok(Self::String(string.to_str()?.to_owned()));
        }
        if let Ok(bool) = obj.cast::<PyBool>() {
            return Ok(Self::Bool(bool.is_true()));
        }
        if let Ok(int) = obj.cast::<PyInt>() {
            return match int.extract::<u32>() {
                Ok(int) => Ok(Self::Integer(int)),
                Err(_) => Ok(Self::Decimal(Decimal::extract(obj)?)),
            };
        }
        if obj.is_instance_of::<PyDate>() {
            return Ok(Self::Date(Date::extract(obj)?));
        }
        if obj.is_instance(get_decimal_decimal(obj.py())?)? {
            return Ok(Self::Decimal(Decimal::extract(obj)?));
        }
        if let Ok(amount) = Amount::extract(obj) {
            return Ok(Self::Amount(amount));
        }
        Err(PyTypeError::new_err(format!(
            "not a valid metadata value: {}",
            obj.get_type()
        )))
    }
}
...
impl EntryMeta {
    ...
    fn get_as_pyany<'py>(&self, key: &str, py: Python<'py>) -> PyResult<Option<Bound<'py, PyAny>>> {
        match key {
            "filename" => (&self.filename).into_bound_py_any(py).map(Some),
            "lineno" => self.lineno.into_bound_py_any(py).map(Some),
...
fn get_keys_view(py: Python<'_>) -> PyResult<&Bound<'_, PyType>> {
    static KEYS_VIEW: PyOnceLock<Py<PyType>> = PyOnceLock::new();
    KEYS_VIEW.import(py, "collections.abc", "KeysView")
}
/// Import collections.abc.ValuesView
fn get_values_view(py: Python<'_>) -> PyResult<&Bound<'_, PyType>> {
        ...
        let meta = obj
            ...
            .map(|(k, v)| {
                ...
                match key.as_str() {
                    ...
                    _ if v.is_none() => Ok(Some(MetaKeyValuePair::new(key, None))),
