...
/// methods defined belows to update the state along the way.
    ...
    fn balance(&mut self, entry: &'ledger Balance) {
        ...
        if let Some(pad) = &self.active_pad {
            if diff.abs() > balance_tolerance(entry, &self.ledger.options) && !padded_already {
                ...
                self.balance.add_amount(diff_units);
            }
        }
    }
}
            ...
            Entry::Transaction(e) => {
                for posting in &e.postings {
                    ...
                    for ancestor in active_ancestors {
                        account_padders.get_mut(ancestor).unwrap().posting(posting);
                    }
                }
            }
            Entry::Pad(e) => {
